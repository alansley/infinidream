...
if cache_valid "${OPENSSL_MARKER}" "${OPENSSL_TOOLCHAIN_MARKER}"; then
    ...
    make install_dev   # headers + shared libs + pkgconfig
...
#   libicu{uc,i18n,data} — ICU, pulled in by Arch's Boost.Log; static Boost
#                   eliminates this dep; excluded here for safety
#
# OpenSSL and FFmpeg are source-built and glibc-2.35 clean; they are bundled.
# Boost is static-linked into the binary (Boost_USE_STATIC_LIBS=ON).
# ---------------------------------------------------------------------------
...
"${LINUXDEPLOY}" \
    ...
    \
    --exclude-library "libicuuc.so*"          \
    --exclude-library "libicui18n.so*"        \
    --exclude-library "libicudata.so*"
